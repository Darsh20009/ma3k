...
// PayPal configuration is optional for development
let isPayPalConfigured = false;

if (!PAYPAL_CLIENT_ID) {
  console.warn("PayPal not configured: Missing PAYPAL_CLIENT_ID");
} else if (!PAYPAL_CLIENT_SECRET) {
  console.warn("PayPal not configured: Missing PAYPAL_CLIENT_SECRET");
} else {
  isPayPalConfigured = true;
  console.log("PayPal integration configured successfully");
}
// Initialize PayPal client only if properly configured
  ...
  try {
    if (!isPayPalConfigured || !ordersController) {
      return res.status(503).json({ 
        error: "PayPal service is not configured. Please contact administrator." 
      });
    }
...
export async function loadPaypalDefault(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({ 
      error: "Failed to initialize PayPal service.",
      clientToken: null 
    });
  }
}
